...
FORMAT_CONFIG = {
    'rl': {
        'train': [
            ...
            ('critic_loss', 'CR_LOSS', 'float'), ('model_loss', 'M_LOSS', 'float')
        ],
...
class Logger(object):
    def __init__(self, log_dir, use_tb=True, config='rl'):
        ...
        if use_tb:
            ...
            self._sw = None
        self._train_mg = MetersGroup(os.path.join(log_dir, 'train.jsonl'), formating=FORMAT_CONFIG[config]['train'])
        self._eval_mg = MetersGroup(os.path.join(log_dir, 'eval.jsonl'), formating=FORMAT_CONFIG[config]['eval'])
